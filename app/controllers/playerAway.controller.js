...
exports.swapTeams = async (req, res) => {
  ...
  try {
    // Fetch all teams
    const teams = await PlayerAway.find();
...
const swapTeamsArray = async (teams, team1Id, team2Id) => {
  ...
  await PlayerAway.deleteMany({}); // Remove all teams from the database
  await PlayerAway.insertMany(teams); // Insert the updated teams array
